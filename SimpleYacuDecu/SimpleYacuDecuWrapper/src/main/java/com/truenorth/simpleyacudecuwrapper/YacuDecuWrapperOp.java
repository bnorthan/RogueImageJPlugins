...
/**
 * Op that calls Yacu Decu deconvolution
 * @author bnorthan
 *
 * @param <T>
 * @param <S>
...
{	
	...
	public void run()
	{
		...
		Img<T> flippedKernel = SimpleYacuDecuWrapperUtilities.FlipPSFQuadrants(kernel, kernel.factory(), dimensions);
	
		// convert Imgs to arrays (TODO: research better way to do this)
		float[] inputBuffer=SimpleYacuDecuWrapperUtilities.convertImageToFloatBuffer(input);
		float[] kernelBuffer=SimpleYacuDecuWrapperUtilities.convertImageToFloatBuffer(flippedKernel);	
		
		// declare memory for output buffer
    	...
    	output=SimpleYacuDecuWrapperUtilities.convertFloatBufferToImage(outputBuffer, dimensions, input.factory(), input.firstElement());
    }
